...
/***/ function(module, exports, __webpack_require__) {
	...
	var App = exports.App = {
	  run: function run() {
	    var store = new _store2.default('Dailymotion Recorder');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _dailymotionHelper = __webpack_require__(7);

	var _dailymotionHelper2 = _interopRequireDefault(_dailymotionHelper);
	...
	var Controller = function () {
	  ...
	  _createClass(Controller, [{
	    ...
	    value: function removeItem(itemId) {
	      ...
	      this.store.removeRecording(itemId).then(function () {
	        _this5.showItemList();
	      }).catch(this.handleError.bind(this));
	    }
	  }, {
	    key: 'downloadItem',
	    value: function downloadItem(itemId) {
	      var _this6 = this;

	      this.store.getRecording(itemId).then(function (item) {
	        _this6.view.downloadItem(item);
	      }).catch(this.handleError.bind(this));
	    }
	  }, {
	    key: 'uploadItem',
	    value: function uploadItem(itemId) {
	      var _this7 = this;

	      this.store.getRecording(itemId).then(function (item) {
	        return _dailymotionHelper2.default.upload(item);
	      }).then(function (response) {
	        console.log('Upload done:', response);
	        return _this7.store.updateRecording(itemId, {
	    ...
	    key: 'watchItem',
	    value: function watchItem(itemId) {
	      var _this8 = this;

	      this.store.getRecording(itemId).then(function (item) {
	        _this8.view.watchRecording(item);
	      }).catch(this.handleError.bind(this));
	    }
	  }, {
	    key: 'watchViewBack',
	    value: function watchViewBack() {
	    ...
	    value: function login() {
	      _dailymotionHelper2.default.login().then(this.view.updateSession.bind(this.view)).catch(this.view.updateSession.bind(this.view));
	    }
	  }, {
	    key: 'logout',
	    value: function logout() {
	      _dailymotionHelper2.default.logout().then(this.view.updateSession.bind(this.view)).catch(this.view.updateSession.bind(this.view));
	    }
	  }, {
	    key: 'handleError',
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ERR_NOT_LOGGED_IN = 'Not logged in';
	var ERR_MISSING_PERMISSIONS = 'Missing permissions';

	var dmAPI = function () {
	  function dmAPI() {
	    _classCallCheck(this, dmAPI);

	    this._dmAsyncInit = null;
	  }

	  _createClass(dmAPI, [{
	    key: '_handleSessionUpdate',
	    value: function _handleSessionUpdate(resolve, reject, resp) {
	      if (!resp.session) {
	    ...
	    value: function getLoginStatus() {
	      ...
	      return new Promise(function (resolve, reject) {
	        ...
	        DM.getLoginStatus(_this._handleSessionUpdate.bind(_this, resolve, reject));
	      });
	    }
	  }, {
	    key: 'init',
	    value: function init() {
	      ...
	      if (!this._dmAsyncInit) {
	        ...
	        this._dmAsyncInit = new Promise(function (resolve, reject) {
	          ...
	          sdk.onerror = function (err) {
	            ...
	            var sdk = document.getElementById('dm-sdk');
	            sdk.parentElement.removeChild(sdk);
	            _this2._dmAsyncInit = null;
	            delete window.dmAsyncInit;
	            reject(err);
	          };
	        ...
	        .then(function () {
	          ...
	          DM.init({
	            apiKey: ("fe6161d88025191dcaf7"), // DM_API_KEY must be defined as an environment variable
	    ...
	    value: function login() {
	      ...
	      return new Promise(function (resolve, reject) {
	        ...
	        DM.login(_this3._handleSessionUpdate.bind(_this3, resolve, reject), {
	          scope: 'read write'
	        });
	      });
	    }
	    ...
	    value: function logout() {
	      ...
	      return new Promise(function (resolve, reject) {
	        ...
	        DM.logout(_this4._handleSessionUpdate.bind(_this4, resolve, reject));
	      });
	    }
	  }, {
	    key: 'getUploadURL',
	    ...
	    key: '_uploadFile',
	    value: function _uploadFile(item, resp) {
	      console.log('[SDK] Uploading file', resp);
	      var upload_url = resp.upload_url;
	    ...
	    value: function api(_ref) {
	      ...
	      return new Promise(function (resolve, reject) {
	        ...
	        DM.api(path, method, params, function (response) {
	          if (!response) {
	            reject('No response');
	    ...
	    key: 'publishVideo',
	    value: function publishVideo(title, upload_response) {
	    ...
	    value: function upload(item) {
	      ...
	      .catch(function (err) {
	        if (err.message === ERR_NOT_LOGGED_IN) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Store = function () {
	  ...
	  _createClass(Store, [{
	    ...
	    key: 'getRecording',
	    value: function getRecording(id) {
	      return this.getRecordings().then(function (recordings) {
	    ...
	    key: 'addRecording',
	    value: function addRecording(_ref) {
	      var id = _ref.id,
	          blob = _ref.blob,
	          _ref$dailymotion_id = _ref.dailymotion_id,
	          dailymotion_id = _ref$dailymotion_id === undefined ? null : _ref$dailymotion_id;
	    ...
	    key: 'updateRecording',
	    value: function updateRecording(id, _ref2) {
	      var _ref2$blob = _ref2.blob,
	          blob = _ref2$blob === undefined ? null : _ref2$blob,
	          _ref2$dailymotion_id = _ref2.dailymotion_id,
	          dailymotion_id = _ref2$dailymotion_id === undefined ? null : _ref2$dailymotion_id;

	      return this.getRecordings().then(function (recordings) {
	        return _localforage2.default.setItem('recordings', recordings.map(function (recording) {
	          if (recording.id === id) {
	    ...
	    key: 'removeRecording',
	    value: function removeRecording(id) {
	      return this.getRecordings().then(function (recordings) {
	...
	var View = function () {
	  ...
	  _createClass(View, [{
	    ...
	    value: function setMediaStream(mediaStream) {
	      ...
	      this.videoRecElem.onloadedmetadata = function () {
	        _this.videoRecElem.play();
	        _this._scaleVideoElem(_this.videoRecElem);
	        _this.recordBtn.removeAttribute('disabled');
	      };
	    }
	  }, {
	    key: 'unsetMediaStream',
	    value: function unsetMediaStream() {
	      this.videoRecElem.pause();
	      this.videoRecElem.srcObject = null;
	      this.recordBtn.setAttribute('disabled', 'disabled');
	    }
	  }, {
	    key: 'setRecordingStarted',
	    value: function setRecordingStarted() {
	      this.showListBtn.setAttribute('disabled', 'disabled');
	      this.recordBtn.classList.add('recording');
	    }
	  }, {
	    key: 'setRecordingStopped',
	    value: function setRecordingStopped() {
	      this.showListBtn.removeAttribute('disabled');
	      this.recordBtn.classList.remove('recording');
	    }
	  }, {
	    key: 'watchRecording',
	    value: function watchRecording(item) {
	      var _this2 = this;
	      ...
	      this.videoWatchElem.onloadedmetadata = function () {
	        _this2._scaleVideoElem(_this2.videoWatchElem);
	        _this2.videoWatchElem.play();
	      };
	    }
	  }, {
	    key: 'clearVideoWatch',
	    value: function clearVideoWatch() {
	    ...
	    key: 'downloadItem',
	    value: function downloadItem(item) {
	    ...
	    key: '_scaleVideoElem',
	    value: function _scaleVideoElem(videoElement) {
	      var elemAspectRatio = videoElement.offsetHeight / videoElement.offsetWidth;
	    ...
	    key: '_catchUserAction',
	    value: function _catchUserAction(videoElement) {
	      videoElement.play().catch(function () {});
	      videoElement.pause();
	    }
	  }, {
	    key: 'bindShowItemList',
	    value: function bindShowItemList(handler) {
	      this.showListBtn.addEventListener('click', function () {
	        handler();
	      });
	    }
	  }, {
	    key: 'bindListViewBack',
	    value: function bindListViewBack(handler) {
	      this.listViewBackBtn.addEventListener('click', function () {
	        handler();
	      });
	    }
	  }, {
	    key: 'bindToggleRecording',
	    value: function bindToggleRecording(handler) {
	    ...
	    key: 'bindWatchViewBack',
	    value: function bindWatchViewBack(handler) {
	      this.watchViewBackBtn.addEventListener('click', function () {
	        handler();
	      });
	    }
	  }, {
	    key: 'bindLoginActions',
	    value: function bindLoginActions(handlers) {
	      var login = handlers.login,
